...
if (!class_exists('MCProtectIpstoreFS_V644')) :
	class MCProtectIpstoreFS_V644 {
		...
		function __construct() {
			...
			$ips = MCProtectUtils_V644::parseFile($ip_store_file);
		...
		public function getTypeIfBlacklistedIP($ip) {
			return $this->getIPType($ip, MCProtectIpstoreFS_V644::IP_TYPE_BLACKLISTED);
		}

		public function isFWIPBlacklisted($ip) {
			return $this->checkIPPresent($ip, MCProtectIpstoreFS_V644::IP_TYPE_BLACKLISTED);
		}

		public function isFWIPWhitelisted($ip) {
			return $this->checkIPPresent($ip, MCProtectIpstoreFS_V644::IP_TYPE_WHITELISTED);
		}
		...
		private function getIPType($ip, $type) {
			switch ($type) {
			case MCProtectIpstoreFS_V644::IP_TYPE_BLACKLISTED:
				return isset($this->blacklisted_ips[$ip]) ? $this->blacklisted_ips[$ip] : null;
			case MCProtectIpstoreFS_V644::IP_TYPE_WHITELISTED:
