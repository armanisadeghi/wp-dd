...
class Content_Protection {
	...
	public function register_controls( $element ) {
		...
		$element->add_control(
			'eael_cp_tab_id',
			[
				'label'       => esc_html__( 'Tab ID', 'essential-addons-elementor' ),
				'type'        => Controls_Manager::TEXT,
				'placeholder' => '#tab-title-2',
				'separator'   => 'before',
				'label_block' => true,
				'ai'          => [
					'active' => false,
				...
				'description' => __( 'If you use this inside tab. Enter the tab ID you want to open after successful unlock.', 'essential-addons-elementor' ),
			]
		);
		...
		$element->add_responsive_control(
			...
			[
				...
				'default'     => 'left',
				'selectors'   => [
					'{{WRAPPER}} .eael-password-protected-content-fields > form' => 'justify-content: {{VALUE}}; display: flex;',
				],
				'condition'   => [
					'eael_ext_content_protection_type' => 'password',
				],
	...
	public function password_protected_form( $widget_id, $settings ) {
		$html = '<div class="eael-password-protected-content-fields">
            <form action="' . ( !empty( $settings[ 'eael_cp_tab_id' ] ) ? esc_attr( $settings[ 'eael_cp_tab_id' ] ) : '' ) . '" method="post">
	...
	public function render_content( $content, $widget ) {
		...
		if( is_bool( $content ) ){
			...
			$widget->add_render_attribute( '_wrapper', [
				'class'             => 'elementor-element elementor-element-' . $widget_id,
				'data-id'           => $widget_id,
				'data-element_type' => $widget->get_name()
			] );
			echo '<div ' . $widget->get_render_attribute_string( '_wrapper' ) . '>' . $html . '</div>';
			return false;
		}
		return $html;
	}
