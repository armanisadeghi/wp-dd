...
wp_register_ability('tpae/tpae-ninja-form', [
    'label' => __('Ninja Form', 'tpebl'),
    'description' => __('Adds the The Plus "Ninja Form" widget (tp-ninja-form) to an Elementor container.', 'tpebl'),
    'category' => 'tpae',
    'input_schema' => [
        'type' => 'object',
        'properties' => [
            ...
            'contact_form' => ['type' => 'integer', 'description' => 'Ninja Forms form ID to embed'],
    ...
    'execute_callback' => 'tpae_mcp_add_theplus_ninja_form_ability',
    'permission_callback' => 'tpae_mcp_add_theplus_ninja_form_permission',
    'meta' => ['show_in_rest' => true, 'mcp' => ['public' => true, 'type' => 'tool']],
]);

function tpae_mcp_add_theplus_ninja_form_permission(?array $input = null): bool {
...
function tpae_mcp_add_theplus_ninja_form_ability(array $input) {
    if (!tpae_mcp_has_elementor()) { return new WP_Error('elementor_missing', __('Elementor must be active.', 'tpebl')); }
    $widget_type = 'tp-ninja-form';
    if (!tpae_mcp_has_registered_widget($widget_type)) { return new WP_Error('widget_missing', __('The Plus Ninja Form widget is not registered on this site.', 'tpebl')); }
    $post_id = absint($input['post_id'] ?? 0);
