...
wp_register_ability('tpae/tpae-table', [
    'label' => __('Table', 'tpebl'),
    'description' => __('Adds the The Plus "Table" widget (tp-table) to an Elementor container.', 'tpebl'),
    'category' => 'tpae',
    ...
    'execute_callback' => 'tpae_mcp_add_theplus_table_ability',
    'permission_callback' => 'tpae_mcp_add_theplus_table_permission',
    'meta' => ['show_in_rest' => true, 'mcp' => ['public' => true, 'type' => 'tool']],
]);

function tpae_mcp_add_theplus_table_permission(?array $input = null): bool {
...
function tpae_mcp_add_theplus_table_ability(array $input) {
    if (!tpae_mcp_has_elementor()) { return new WP_Error('elementor_missing', __('Elementor must be active.', 'tpebl')); }
    $widget_type = 'tp-table';
    if (!tpae_mcp_has_registered_widget($widget_type)) { return new WP_Error('widget_missing', __('The Plus Table widget is not registered.', 'tpebl')); }
    $post_id = absint($input['post_id'] ?? 0);
    ...
    $settings = [];
    if (!empty($input['table_selection'])) { $settings['table_selection'] = sanitize_key((string)$input['table_selection']); }
