...
wp_register_ability('tpae/tpae-everest-form', [
    'label' => __('Everest Form', 'tpebl'),
    'description' => __('Adds the The Plus "Everest Form" widget (tp-everest-form) to an Elementor container.', 'tpebl'),
    'category' => 'tpae',
    'input_schema' => [
        'type' => 'object',
        'properties' => [
            ...
            'everest_form' => ['type' => 'integer', 'description' => 'Everest Forms form ID to embed'],
        ...
        'required' => ['post_id', 'parent_id', 'everest_form'],
        'additionalProperties' => false,
    ],
    'output_schema' => ['type' => 'object', 'properties' => ['element_id' => ['type' => 'string'], 'widget_type' => ['type' => 'string'], 'post_id' => ['type' => 'integer']]],
    'execute_callback' => 'tpae_mcp_add_theplus_everest_form_ability',
    'permission_callback' => 'tpae_mcp_add_theplus_everest_form_permission',
    'meta' => ['show_in_rest' => true, 'mcp' => ['public' => true, 'type' => 'tool']],
]);

function tpae_mcp_add_theplus_everest_form_permission(?array $input = null): bool {
...
function tpae_mcp_add_theplus_everest_form_ability(array $input) {
    if (!tpae_mcp_has_elementor()) { return new WP_Error('elementor_missing', __('Elementor must be active.', 'tpebl')); }
    $widget_type = 'tp-everest-form';
    if (!tpae_mcp_has_registered_widget($widget_type)) { return new WP_Error('widget_missing', __('The Plus Everest Form widget is not registered on this site.', 'tpebl')); }
    $post_id = absint($input['post_id'] ?? 0);
    ...
    $settings = [];
    if (!empty($input['everest_form'])) { $settings['everest_form'] = absint($input['everest_form']); }
